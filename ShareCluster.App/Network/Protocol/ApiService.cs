...
namespace ShareCluster.Network.Protocol
{
    ...
    public class ApiService : IApiService
    {
        ...
        public CatalogDataResponse GetCatalog(CatalogDataRequest request)
        {
            ...
            var upToDate = actualVersion == request.KnownCatalogVersion;
        ...
        private (DataResponseSuccess, DataResponseFault) GetDataStreamWithSlot(DataRequest request)
        {
            ...
            if (!package.DownloadStatus.TryCreateOfferForPeer(request.DownloadedSegmentsBitmap, _networkSettings.SegmentsPerRequest, out var segments))
            {
                return (null, DataResponseFault.CreateDataPackageSegmentsNoMatchMessage());
            }

            // obtain lock
            if (!package.Locks.TryObtainSharedLock(out var lockToken))
            {
                return (null, DataResponseFault.CreateDataPackageNotFoundMessage());
            }
